         ...
         if (frame->linesize[0] > 0 && frame->linesize[1] > 0 &&
         ...
         else
            {
               av_log(NULL, AV_LOG_ERROR,
                      "Mixed negative and positive linesizes are not "
                      "supported.\n");
               return -1;
            }
         break;
                           ...
                           for (i = 0; i < sp->sub.num_rects; i++)
                              {
                                 ...
                                 sub_rect->x =
                                     av_clip(sub_rect->x, 0, sp->width);
                                 sub_rect->y =
                                     av_clip(sub_rect->y, 0, sp->height);
                                 sub_rect->w = av_clip(sub_rect->w, 0,
                                                       sp->width - sub_rect->x);
                                 sub_rect->h = av_clip(
                                     sub_rect->h, 0, sp->height - sub_rect->y);

                                 is->sub_convert_ctx = sws_getCachedContext(
                                     is->sub_convert_ctx, sub_rect->w,
                                     sub_rect->h, AV_PIX_FMT_PAL8, sub_rect->w,
                                     sub_rect->h, AV_PIX_FMT_BGRA, 0, NULL,
                                     NULL, NULL);
         ...
         if (!last_time || (cur_time - last_time) >= 30000)
            {
               ...
               av_log(
                   ...
                   is->video_st
                       ? is->viddec.avctx->pts_correction_num_faulty_dts
                       : 0,
                   is->video_st
   ...
   for (;;)
      {
         ...
         if ((got_subtitle =
                  decoder_decode_frame(&is->subdec, NULL, &sp->sub)) < 0)
            break;
               ...
               if (event.button.button == SDL_BUTTON_LEFT)
                  {
                     ...
                     if (av_gettime_relative() - last_mouse_left_click <=
                         500000)
                        {
                           toggle_full_screen(cur_stream);
                           cur_stream->force_refresh = 1;
               ...
               if (seek_by_bytes || cur_stream->ic->duration <= 0)
                  {
                     ...
                     stream_seek(cur_stream, size * x / cur_stream->width, 0,
                                 1);
                  }
               else
                  {
         ...
         if (window)
            {
               ...
               if (!renderer)
                  {
                     av_log(NULL, AV_LOG_WARNING,
                            "Failed to initialize a hardware accelerated "
                            "renderer: %s\n",
                            SDL_GetError());
